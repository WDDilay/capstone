<template>
  <div class="bg-gradient-to-br from-surface-50 to-surface-100 min-h-screen overflow-hidden">
    <div class="max-w-7xl mx-auto p-6 md:p-8">
            ...
            <div class="custom-calendar">
              ...
              <div class="calendar-days">
                ...
                >
                  ...
                  <!-- Events for this day (ONLY from the 5 kept for calendar) -->
                  <div class="day-events">
                    ...
                    <!-- Show +X more if there are more than 3 events for that day (within the 5-calendar cap set) -->
                    <div 
      ...
      <!-- More Events Dialog (for >3 on the SAME DAY within the 5-calendar set) -->
      <Dialog 
...
import { format, isSameDay, isAfter, startOfDay, addMonths, subMonths, getDate, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isToday as dateFnsIsToday, startOfWeek, endOfWeek } from 'date-fns';
...
// Permissions
const canAddEvents = computed(() => user.value?.role === 'FederationPresident' || user.value?.role === 'BarangayPresident');
const isCurrentUserFederationPresident = computed(() => user.value?.role === 'FederationPresident');
const isFederationPresidentEvent = computed(() => newEvent.value.createdBy === 'FederationPresident');

const isReadOnlyMode = computed(() => {
  if (isFederationPresidentEvent.value && !isCurrentUserFederationPresident.value) return true;
  if (editMode.value && !canEditEvent(newEvent.value)) return true;
  return false;
});

const canEditEvent = (event) => {
  if (event.createdBy === 'FederationPresident') return user.value?.role === 'FederationPresident';
  if (user.value?.role === 'BarangayPresident') return event.createdBy === 'BarangayPresident' && event.barangay === user.value?.barangay;
  return false;
};
...
/**
 * PROCESSING LOGIC
 * - Remove past events (only today & future).
 * - Sort ascending by date.
 * - First 5 => shown on calendar.
 * - Overflow => Upcoming list.
 */
...
// For a specific day, only use the 5 calendar-kept events
const getDayEvents = (day) => {
  return processedEvents.value.calendar.filter(event => isSameDay(new Date(event.date), day));
};

// More events dialog for a single day
...
// Upcoming list (overflow beyond the 5 kept on calendar)
const upcomingEvents = computed(() => processedEvents.value.upcoming);

// Keep selected date events in sync (again, only from the 5 calendar-kept)
function updateSelectedDateEvents() {
  selectedDateEvents.value = processedEvents.value.calendar.filter(event => 
    isSameDay(new Date(event.date), date.value)
  );
}

watch([date, processedEvents], updateSelectedDateEvents, { immediate: true });

// Fetch events (initial) - keep raw list; UI will apply the 5-cap/past-filter
const fetchEvents = async () => {
  ...
  querySnapshot.forEach((docu) => {
    ...
    // Client-side filter for barangay where needed:
    if (user.value?.role === 'BarangayPresident') {
      if (data.createdBy === 'BarangayPresident' && data.barangay !== user.value?.barangay) return;
  ...
  events.value = list;
  nextTick(() => console.log('Calendar events updated'));
};
...
const openNewEventDialog = () => {
  newEvent.value = { 
    ...
    barangay: user.value?.role === 'BarangayPresident' ? (user.value?.barangay || '') : (newEvent.value.barangay || '')
  };
...
const saveEvent = async () => {
  ...
  try {
    if (editMode.value) {
      ...
      const eventsCollection = collection(db, 'announcements');
      const barangayValue = user.value?.role === 'BarangayPresident' ? (user.value?.barangay || '') : '';
...
// Preserve time when changing just the date
...
/* PrimeVue datepicker styling (from your original) */
:deep(.p-datepicker) {
...
:deep(.p-datepicker-calendar) { font-size: 0.95rem; }
:deep(.p-datepicker-calendar th) { color: #64748b; font-weight: 600; padding: 0.5rem; }
:deep(.p-datepicker-calendar td) { padding: 0.25rem; position: relative; }
...
.barangay-input, .creator-input { background-color: #f1f5f9; color: #64748b; }
